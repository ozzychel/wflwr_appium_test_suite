import { USER_EMAIL, USER_FIRSTNAME, USER_LASTNAME, USER_PASSWORD, DEFAULT_PIN, ANDROID_APP_NAME, IOS_APP_NAME } from '../helpers/Constants';
...
describe('WFLWR E2E AUTOMATION TEST RUNNER', () => {
  ...
  describe('LOGIN SCREEN. EMAIL FLOW. SIGN UP AS A NEW USER', () => {
    ...
    //android 13 devices show up Notifications permission dialog (not happening on android 12)
    // to handle this will run one more test
    it('ALLOW app to send notifications IF dialog SHOWS UP', async () => {
      const flag = await PermissionsDialog.dialog.isDisplayed();
      if (flag) {
